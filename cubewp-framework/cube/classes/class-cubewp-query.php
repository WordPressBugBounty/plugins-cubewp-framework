    ...
    public static function cubewp_query_builder()
    {
        ...
        if (isset($args['orderby']) && !empty($args['orderby'])) {
            if (substr($args['orderby'], -strlen('-DESC')) === '-DESC') {
                ...
                // Handle reviews plugin sorting options
                $reviews_active = defined('CUBEWP_REVIEWS') || class_exists('CubeWp_Reviews_Load');
                if ($reviews_active) {
                    // Most viewed - check for post views meta
                    if ($args['orderby'] === 'post_views') {
                        // Try to find post views meta key
                        $post_views_key = 'cubewp_post_views';
                        // Check if meta exists, otherwise use default
                        $query['meta_key'] = $post_views_key; // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
                        $query['orderby'] = 'meta_value_num';
                        $query['order'] = 'DESC';
                    }
                    // High rated - will be sorted by calculated rating in filter
                    elseif ($args['orderby'] === 'average_rating') {
                        // Rating sorting will be handled by post__in filter
                        // Just set a default order
                        $query['orderby'] = 'date';
                        $query['order'] = 'DESC';
                    }
                    // Rating X - will be filtered in filter_ratings_and_views
                    elseif (strpos($args['orderby'], 'rating_') === 0) {
                        // Filtering handled in filter_ratings_and_views
                        $query['orderby'] = 'date';
                        $query['order'] = 'DESC';
                    }
                    // Standard orderby
                    ...
                    // No reviews plugin, standard handling
                    if (isset($args['orderby']) && ($args['orderby'] == 'DESC' || $args['orderby'] == 'ASC')) {
                        $query['order'] = $args['orderby'];
                    } else {
                        $query['orderby'] = $args['orderby'];
